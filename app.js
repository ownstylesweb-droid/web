...
if (!clientEmail || !clientEmail.includes("@")) {
  ...
  emailjs.send("CamChaves27", "template_rqetchq", data)
    .then(() => {
      alert("¡Tu pedido fue enviado correctamente! Revisa tu correo.");
...
document.getElementById("contactForm").addEventListener("submit", function(e) {
  ...
  if (!message) { alert("Por favor, escribe tu mensaje."); return; }

  const templateParams = { name, email, message, time };

  emailjs.send("CamChaves27", "template_ru5b7oe", templateParams, "bEyf2tJ4yNiFw2N4k")
    .then(() => {
